...
#'
#' @examples
#' vcfFile <- system.file("extdata", "large_demo.vcf.gz", package = "vcfFilterPlot")
#' gdsFile <- system.file("extdata", "large_demo.vcf.gz", package = "vcfFilterPlot")
...
openGDSFile <- function(vcfFilePath, gdsFilePath) {
  ...
  if (!file.exists(gdsFilePath)) {
    ...
    message("Converting VCF to GDS format. Subsequent filtering and plotting on this data will be much faster after one-time conversion to GDS.")
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # Default QUAL threshold is 100 which is  0.00000001% chance of incorrect variant call
...
server <- function(input, output, session) {
  ...
  observeEvent(input$runAnalysis, {
    ...
    # This can be replacing genotypes with gq under threshold with ./. or NA
    #
